...
class Bot {
    ...
    async sendWelcomeMessage(chatId, userLang) {
        const welcomeMsg = await translateText("Welcome! Use 'set language [language]' to change the language. Send a number (between 1-101) to receive a joke. For example, '5' to get the fifth joke.", userLang);
